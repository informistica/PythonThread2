...
def producer(producer_id):
    while True:
        ...
        print(f"Produttore {producer_id} ha prodotto il dato {data} e rilasciato il semaforo [num elem nella coda {buffer.qsize()}]-> [{list(buffer.queue)}]")
...
il comportamento dipende dall'argomento block.
Se block è True (il valore predefinito), il metodo attende finché non c'è spazio disponibile 
nella coda. Se block è False, viene sollevata un'eccezione queue.Full se la coda è piena. 
L'argomento opzionale timeout specifica il tempo massimo di attesa (in secondi) per l'operazione 
di inserimento.
.get([block[, timeout]]): Rimuove e restituisce un elemento dalla coda. 
Se la coda è vuota, il comportamento dipende dall'argomento block. 
Se block è True (il valore predefinito), il metodo attende finché non ci sono elementi disponibili 
nella coda. Se block è False, viene sollevata un'eccezione queue.Empty se la coda è vuota. 
L'argomento opzionale timeout specifica il tempo massimo di attesa (in secondi) per l'operazione 
di rimozione.
...
.join(): Blocca il thread chiamante finché tutti gli elementi presenti nella coda non vengono elaborati. 
Questo metodo è utile in combinazione con il threading per aspettare il completamento di tutti i 
task inseriti nella coda.
